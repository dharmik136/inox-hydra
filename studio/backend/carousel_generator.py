...
def get_font(size: int, bold: bool = False):
    """Fallback font loader with system font discovery."""
    font_paths = [
...
def render_slide(
    ...
    theme = THEMES.get(theme_name, THEMES["dark_slate"])
    ...
    draw.text((130, 64), author_name, fill=theme["text"], font=font_author)

    font_title = get_font(15, bold=False)
    draw.text((130, 92), author_title, fill=theme["text_muted"], font=font_title)
    ...
    if is_cover:
        ...
        title_lines = wrap_text(title, font_main_title, content_width, draw)
        ...
        body_lines = wrap_text(body, font_body, content_width, draw)
    ...
    prog_width = int((WIDTH - 128) * ((slide_index + 1) / total_slides))
