...
def test_sync_refuses_a_disallowed_host_without_making_a_request(monkeypatch):
    ...
    def explode(*args, **kwargs):
        calls.append(args)
        raise AssertionError("a request was made to a disallowed host")

    monkeypatch.setattr(intelligence_sync.urllib.request, "urlopen", explode)
...
def test_the_scheme_rule_still_applies(monkeypatch):
    ...
    monkeypatch.setattr(
        intelligence_sync.urllib.request, "urlopen",
